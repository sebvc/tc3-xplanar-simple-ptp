 ...
 * @param transformObject object to test
 * @preserve (Part of the public API)
 */export function isPerspective(transformObject){return!!transformObject&&"Perspective"===transformObject.transformType}export function isComparison(value){if(!value)return!1;return["==","!=","<",">","<=",">=","contains","contains not","== [ignore case]","!= [ignore case]","contains [ignore case]","contains not [ignore case]"].includes(value.comparator)&&(["string","number","boolean"].includes(typeof value.value)||value.value instanceof Date||null===value.value)}export function isLogicOperator(value){if(!value)return!1;let logic=value.logic;return"AND"===logic||"OR"===logic}