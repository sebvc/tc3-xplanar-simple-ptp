...
(function (TcHmi) {
    ...
    (function (Controls) {
        let XPlanarView;
        (function (XPlanarView_1) {
            class XPlanarView extends TcHmi.Controls.System.TcHmiControl {
                /*
                ...
                */
                ...
                // internal
                __vectorSubscriptionId;
                __moverImg;
                __moverAnimationFrameId;
                __moverPrevPositions;
                __moverAnimStartTime = 0;
                __moverAnimFrom;
                __moverAnimTo;
                // styling
                __fontSize;
                ...
                */
                ...
                __createCanvas() {
                    ...
                    this.__tileCtx = this.__tileCanvas.getContext('2d');
                    this.__moverCtx = this.__moverCanvas.getContext('2d');
                    this.__stationCtx = this.__stationCanvas.getContext('2d');
                ...
                setTileConfig(valueNew) {
                    // check if the value is valid
                    if (valueNew === null) {
                        // if we have no value to set we have to fall back to the defaultValueInternal from description.json
                        valueNew = this.getAttributeDefaultValueInternal('TileConfig');
                    }
                ...
                __handleMoverLayout(moverPositions) {
                ...
                setMoverInfo(valueNew) {
                    // check if the value is valid
                    if (valueNew === null) {
                        // if we have no value to set we have to fall back to the defaultValueInternal from description.json
                        valueNew = this.getAttributeDefaultValueInternal('MoverInfo');
                    }
                ...
                __subscribeVectorData() {
                    ...
                    let commands = [
                        {
                            'symbol': this.__moverVectorSymbol?.getExpression().getName()
                        }
                    ];
                ...
                __animateMovers() {
                    ...
                    const interpolated = this.__moverAnimFrom.map((from, i) => {
                ...
                __setMoverVectorData(valueNew) {
                    // check if the value is valid
                    if (valueNew === null) {
                        // if we have no value to set we have to fall back to the defaultValueInternal from description.json
                        valueNew = this.getAttributeDefaultValueInternal('MoverVectorData');
                    }
                ...
                setRefreshRate(valueNew) {
                    // check if the value is valid
                    if (valueNew === null) {
                        // if we have no value to set we have to fall back to the defaultValueInternal from description.json
                        valueNew = this.getAttributeDefaultValueInternal('RefreshRate');
                    }
                ...
                __processRefreshRate() {
                    this.__subscribeVectorData();
                }
                setDisplayMoverBTN(valueNew) {
                    // check if the value is valid
                    if (valueNew === null) {
                        // if we have no value to set we have to fall back to the defaultValueInternal from description.json
                        valueNew = this.getAttributeDefaultValueInternal('DisplayMoverBTN');
                    }
                ...
                setDisplayCustomText(valueNew) {
                    // check if the value is valid
                    if (valueNew === null) {
                        // if we have no value to set we have to fall back to the defaultValueInternal from description.json
                        valueNew = this.getAttributeDefaultValueInternal('DisplayCustomText');
                    }
                ...
                getDisplayCustomText() {
                    return this.__displayCustomText;
                }
                setSmoothMotion(valueNew) {
                    // check if the value is valid
                    if (valueNew === null) {
                        // if we have no value to set we have to fall back to the defaultValueInternal from description.json
                        valueNew = this.getAttributeDefaultValueInternal('SmoothMotion');
                    }
                ...
                __processSmoothMotion() {
                    this.__subscribeVectorData();
                }
                setUseMoverImage(valueNew) {
                    // convert the value with the value converter
                    let convertedValue = TcHmi.ValueConverter.toBoolean(valueNew);
                    // check if the converted value is valid
                    if (convertedValue === null) {
                        // if we have no value to set we have to fall back to the defaultValueInternal from description.json
                        convertedValue = this.getAttributeDefaultValueInternal('UseMoverImage');
                    }
                ...
                __processUseMoverImage() {
                    this.__handleMoverLayout();
                }
                /**
                 *  Station handling
                 */
                __handleStationLayout() {
                    //@ts-ignore because tsc isn't aware of reset method
                ...
                setStationLayout(valueNew) {
                    // check if the value is valid
                    if (valueNew === null) {
                        // if we have no value to set we have to fall back to the defaultValueInternal from description.json
                        valueNew = this.getAttributeDefaultValueInternal('StationLayout');
                    }
                 ...
                 *  Canvas scaling
                 */
                __getTrackDimensions() {
                    let minX, minY, maxX, maxY, maxW, maxH;
                ...
                setScaleToFit(valueNew) {
                    // convert the value with the value converter
                    let convertedValue = TcHmi.ValueConverter.toBoolean(valueNew);
                    // check if the converted value is valid
                    if (convertedValue === null) {
                        // if we have no value to set we have to fall back to the defaultValueInternal from description.json
                        convertedValue = this.getAttributeDefaultValueInternal('ScaleToFit');
                    }
                ...
                setRotate(valueNew) {
                    ...
                    if (convertedValue === null) {
                        // if we have no value to set we have to fall back to the defaultValueInternal from description.json
                        convertedValue = this.getAttributeDefaultValueInternal('Rotate');
                    }
                 ...
                 */
                setSelectedMoverIndex(valueNew) {
                    // convert the value with the value converter
                    let convertedValue = TcHmi.ValueConverter.toNumber(valueNew);
                    // check if the converted value is valid
                    if (convertedValue === null) {
                        // if we have no value to set we have to fall back to the defaultValueInternal from description.json
                        convertedValue = this.getAttributeDefaultValueInternal('SelectedMoverIndex');
                    }
                ...
                getSelectedMoverIndex() {
                    return this.__selectedMoverIndex;
                }
                __processSelectedMoverIndex() {
                    this.__handleMoverLayout();
                }
                /**
                 * Font handling
                ...
                setFontSizeUnit(valueNew) {
                    let convertedValue = TcHmi.ValueConverter.toFontSizeUnit(valueNew);
                    if (convertedValue === null) {
                        convertedValue = this.getAttributeDefaultValueInternal("FontSizeUnit");
                    }
            ...
            XPlanarView_1.XPlanarView = XPlanarView;
        })(XPlanarView = Controls.XPlanarView || (Controls.XPlanarView = {}));
...
//# sourceMappingURL=XPlanarView.js.map