...
/**
     ...
     * @template R Type of the value after write. Falls back to W if not specified.
     * @preserve (Part of the public API)
     */writeEx(value,options,callback){const callstackLinker=createTask("Symbol.write>"+this.getExpression());let destroy=this.__symbol.writeEx2(value,options,null,data=>{const result={error:data.error,expression:data.expression,destroy:data.destroy};void 0!==data.expressionResolved&&(result.expressionResolved=data.expressionResolved),void 0!==data.value&&(result.value=data.value),data.details&&(result.details=data.details),Symbol.isIServerWriteResultObject(data)&&(result.response=data.response),callstackLinker.run(()=>{Callback.callSafeEx(callback,this,result)})});return()=>{destroy?.(),destroy=null}}
/**
...
/**
     ...
     * @preserve (Part of the public API)
     */
function writeEx2(expressionOrExpressionObject,value,options,callback){let s=new Symbol(expressionOrExpressionObject),destroy=s.writeEx(value,options,data=>{const result={error:data.error,expression:data.expression,destroy:data.destroy};void 0!==data.expressionResolved&&(result.expressionResolved=data.expressionResolved),void 0!==data.value&&(result.value=data.value),data.details&&(result.details=data.details),isIServerWriteResultObject(data)&&(result.response=data.response),Callback.callSafeEx(callback,null,result),s?.destroy(),s=null});return()=>{destroy?.(),destroy=null}}function isIServerReadResultObject(data){return"response"in data}function isIServerWriteResultObject(data){return"response"in data}Symbol.ObjectResolver=ApiObjectResolver,Symbol.read=
/**
