 ...
 * @param callback Will be called after request.
 * @preserve (Part of the public API)
 */export function uploadRecipeFilesEx(options,requestOptions=null,callback=null){let domain;if(options){let paramInvalid=isParameterTypeInvalid(options.domain,"options.domain",{type:"string",required:"undefinedOk",minStringLength:1},"TcHmi.Server.RecipeManagement",callback);if(paramInvalid)return paramInvalid;if("string"==typeof options.domain&&options.domain.length>0&&(domain=options.domain),paramInvalid=isParameterTypeInvalid(options.parallel,"options.parallel",{type:"boolean",required:"undefinedOk"},"TcHmi.Server.RecipeManagement",callback),paramInvalid)return paramInvalid}let pendingFileCount=0;return FileUploader.openFileDialog({multiple:!0,acceptedFileTypes:[".json"]}).then(files=>{if(files?.length){pendingFileCount+=files.length;for(const file of files)file.text().then(result=>{if(pendingFileCount<=0)return;const request=ValueConverter.toObject(result);if(!request?.commands?.length)return pendingFileCount=0,void Callback.callSafeEx(callback,null,{error:Errors.E_PARAMETER_INVALID,details:{code:Errors.E_PARAMETER_INVALID,message:Errors[Errors.E_PARAMETER_INVALID],reason:"Selected recipe file to upload has invalid content.",domain:"TcHmi.Server.RecipeManagment"}});for(const command of request.commands){if(!command.symbol||!command.symbol.includes(".Config::"))return pendingFileCount=0,void Callback.callSafeEx(callback,null,{error:Errors.E_PARAMETER_INVALID,details:{code:Errors.E_PARAMETER_INVALID,message:Errors[Errors.E_PARAMETER_INVALID],reason:"Selected recipe file to upload has invalid content. Symbol not allowed: "+command.symbol,domain:"TcHmi.Server.RecipeManagment"}});if(domain){const symbol=command.symbol,configIndex=symbol.indexOf(".Config::");command.symbol=domain+symbol.substring(configIndex)}}options?.parallel&&(requestOptions?requestOptions.parallel=!0:requestOptions={parallel:!0}),Server.requestEx(request,requestOptions,data=>{if(!(pendingFileCount<=0)){if(data.error||!data.results)return pendingFileCount=0,void Callback.callSafeEx(callback,null,{error:data.error,details:data.details});for(const res of data.results)if(res.error!==Errors.NONE)return pendingFileCount=0,void Callback.callSafeEx(callback,null,{error:res.error,details:res.details});pendingFileCount--,pendingFileCount<=0&&Callback.callSafeEx(callback,null,{error:Errors.NONE})}})}).catch(()=>{pendingFileCount<=0||(pendingFileCount=0,Callback.callSafeEx(callback,null,{error:Errors.E_PARAMETER_INVALID,details:{code:Errors.E_PARAMETER_INVALID,message:Errors[Errors.E_PARAMETER_INVALID],reason:"Reading recipe file to upload has failed.",domain:"TcHmi.Server.RecipeManagment"}}))})}else pendingFileCount=0,Callback.callSafeEx(callback,null,{error:Errors.NONE})}),{code:Errors.NONE}}export function getRecipeVersioningEnabled(options,requestOptions){let domain="TcHmiRecipeManagement";if(options){let paramInvalid=isParameterTypeInvalid(options.domain,"options.domain",{type:"string",required:"undefinedOk",minStringLength:1},"TcHmi.Server.RecipeManagement");if(paramInvalid)return Promise.reject(new Exception(paramInvalid));if("string"==typeof options.domain&&options.domain.length>0&&(domain=options.domain),paramInvalid=isParameterTypeInvalid(options.parallel,"options.parallel",{type:"boolean",required:"undefinedOk"},"TcHmi.Server.RecipeManagement"),paramInvalid)return Promise.reject(new Exception(paramInvalid))}return options?.parallel&&(requestOptions?requestOptions.parallel=!0:requestOptions={parallel:!0}),new Promise((resolve,reject)=>{const symbol=domain+".Config::recipeVersioningEnabled";null===Server.readSymbolEx(symbol,requestOptions??null,data=>{if(data.error!==Errors.NONE)return void reject(new Exception(data.details??{code:data.error}));if(!data.response)return void reject(new Exception(Errors.E_SERVER_RESPONSE_MISSING,`Missing response from server while trying to read ${symbol}.`,"TcHmi.Server.RecipeManagement"));if(data.response.error)return void reject(new Exception(data.response.error));data.response.commands&&0!==data.response.commands.length||reject(new Exception(Errors.E_SERVER_COMMANDS_MISSING,`Missing commands in server response with id ${data.response.id} while trying to read ${symbol}.`,"TcHmi.Server.RecipeManagement"));const command=data.response.commands[0];if(command.error)return"TcHmiSrv"===command.error.domain&&512===command.error.code?void resolve(!1):void reject(new Exception(command.error));"boolean"==typeof command.readValue?resolve(command.readValue):reject(new Exception(Errors.E_SERVER_READVALUE_MISSING,`Read value is missing or has wrong type in server response with id ${data.response.id} while trying to read ${symbol}.`,"TcHmi.Server.RecipeManagement"))})&&reject(new Exception(Errors.ERROR,"Request could not be sent.","TcHmi.Server.RecipeManagement"))})}export function isRecipe(candidate){return"string"==typeof candidate.recipeTypeName&&"object"==typeof candidate.values&&null!==candidate.values}export function isRecipeType(candidate){if("object"!=typeof candidate.members||null===candidate.members)return!1;for(const member of Object.values(candidate.members))if("string"!=typeof member.recipeType){if("string"!=typeof member.symbol)return!1;if(!("defaultValue"in member))return!1}const additionalChecksRequired=0===Object.keys(candidate.members).length;let recipeTypeNamesValid=!1,optionsValid=!1;if("recipeTypeNames"in candidate){if(!Array.isArray(candidate.recipeTypeNames))return!1;for(const entry of candidate.recipeTypeNames){if("string"!=typeof entry)return!1;recipeTypeNamesValid=!0}}if("options"in candidate){if("object"!=typeof candidate.options||null===candidate.options)return!1;if("enabled"in candidate.options){if("None"!==candidate.options.enabled&&"Disabled"!==candidate.options.enabled)return!1;optionsValid=!0}if("comment"in candidate.options){if("string"!=typeof candidate.options.comment)return!1;optionsValid=!0}}return!(additionalChecksRequired&&!recipeTypeNamesValid&&!optionsValid)}export function isVersionedRecipe(candidate){if(isRecipe(candidate))return!1;const children=Object.values(candidate);return children.length>0&&children.every(child=>isRecipe(child))}