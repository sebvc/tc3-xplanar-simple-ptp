...
export declare class Slave extends Component {
    ...
    private __updateProcessDataForceFlags;
    ...
    private __convertProcessDataValueToHistoryNumber;
    /**
     * Resolves the value of every sub value of a process data entry, keyed by its path. The bits of the safe data are
     * derived from the number they belong to, because the server reports that number instead of a bit array.
     */
    collectSubItemValues(entry: Slave_ProcessDataObjectEntry, structValue?: unknown): Record<string, unknown>;
    private __formatProcessDataValue;
    updateProcessDataValuesOnlineElements(entry: Slave_ProcessDataObjectEntry): boolean;
    /** Applies the current value and force state to the elements of every displayed sub value. */
    private __updateProcessDataPropertyElements;
    addProcessDataValueElement(entry: OnlineEntry<any>, el: HTMLElement): void;
    private __errorMessages;
    get currentStateMachineState(): number | null;
...
export interface SlaveData_Identity {
    ...
    /** ISO date string of the production date, `null` if the slave does not report one. */
    productionDate: OnlineEntry<string | null>;
...
export interface SlaveData_online {
    current: {
        stateMachine: {
            ...
            extendedCode: OnlineEntry<number | null>;
        };
        presence: {
