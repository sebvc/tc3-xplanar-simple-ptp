...
declare class TcHmiCombobox<T extends any = any, D extends ListItemGeneric<T>[] | T[] | Dictionary<T> = ListItemGeneric<T>[] | T[] | Dictionary<T>> extends TcHmiControl.Control {
    ...
    /**
     * Pointer gesture arbitration: when text input is allowed the value field is an editable input. Take ownership of
     * a left-button drag that starts on it so the user can select its text with the mouse instead of an enclosing
     * zoom/pan region taking the drag over as a pan. A deactivated control disables the input element, so text cannot
     * be selected and the drag is left to the region.
