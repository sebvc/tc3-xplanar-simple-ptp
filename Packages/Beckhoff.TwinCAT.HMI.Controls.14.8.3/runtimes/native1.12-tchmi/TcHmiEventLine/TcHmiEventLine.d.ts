...
declare class TcHmiEventLine extends TcHmi.Controls.System.TcHmiControl {
    ...
    /**HTML-ElementTemplate for the row (icon + message) that slides vertically*/
    protected __elementRow: HTMLElement;
    /**HTML-ElementTemplate for the message display*/
    ...
    /**Eventlist */
    protected __events: (Server.Events.Message | Server.Events.Alarm | Server.Events.PayloadEvent)[];
    /** index of the currently displayed event */
    protected __currentIndex: number;
    /** vertical event rotation settings */
    protected __verticalEventRotation: VerticalEventRotation | null | undefined;
    /** handle of the rotation timer */
    protected __rotationTimer: number | null;
    /** cached default rotation time (ms) from the schema */
    protected __defaultRotationTime: number | null;
    ...
    /**
     * Sets the value of the member variable VerticalEventRotation.
     * @param valueNew The new value for VerticalEventRotation
     */
    setVerticalEventRotation(valueNew: VerticalEventRotation | null | undefined): void;
    /**
     * The watch callback for the verticalEventRotation object resolver.
     */
    protected __onResolverForVerticalEventRotationWatchCallback(data: ObjectResolver.IWatchResultObject<VerticalEventRotation | null>): void;
    /**
     * Returns the current value of VerticalEventRotation.
     * @returns The current value of VerticalEventRotation.
     */
    getVerticalEventRotation(): VerticalEventRotation | null | undefined;
    /**
     * Processes the current VerticalEventRotation value.
     */
    protected __processVerticalEventRotation(): void;
    /**
     * Restarts the rotation timer based on the current settings.
     */
    protected __restartRotationTimer(): void;
    /**
     * Stops the rotation timer.
     */
    protected __stopRotationTimer(): void;
    /**
     * Triggers the vertical slide-in animation for the incoming row (icon and message together).
     */
    protected __playSlideAnimation(): void;
    /**
     * Whether a rotation slide animation should be played. Rotation must be enabled and a display
     * time (displayTime) must be set. A rotationTime of 0 switches the event instantly without any
     * animation (useful on slow systems).
     */
    protected __isRotationAnimated(): boolean;
    /**
     * Whether the vertical event rotation is enabled. Rotation defaults to enabled when the
     * VerticalEventRotation object is set but the enabled flag is omitted.
     */
    protected __isRotationEnabled(): boolean;
    /**
     * Reads the default rotation time (switch animation duration in ms) from the schema definition
     * so the default value is only defined once in the JSON files. The result is cached because the
     * schema is static.
     */
    protected __getDefaultRotationTime(): number;
    /**
     * Returns the effective rotation time in ms: the configured rotationTime or the schema default.
     */
    protected __getEffectiveRotationTime(): number;
    /**
     * Removes any slide-out clones left over from a rotation transition.
     */
    protected __removeSlideOutClones(): void;
    /**
     * Clones the current row and slides it out while the next one slides in.
     */
    protected __playSlideOut(): void;
}
...
export interface VerticalEventRotation {
    enabled?: boolean | null;
    rotationTime?: number | null;
    eventCount: number;
    displayTime: number;
}
...
type tVerticalEventRotation = VerticalEventRotation;
declare global {
    namespace TcHmi.Controls.Beckhoff {
        ...
        namespace TcHmiEventLine {
            ...
            type VerticalEventRotation = tVerticalEventRotation;
        }
    }
}
